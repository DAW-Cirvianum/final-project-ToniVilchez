...
import { 
  User, Mail, Globe, Camera, Save, Upload, 
...
export default function Profile() {
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = t('profile.errors.nameRequired', 'El nombre es requerido');
    }
    if (!formData.email.trim()) {
      newErrors.email = t('profile.errors.emailRequired', 'El email es requerido');
    } else if (!formData.email.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)) {
      newErrors.email = t('profile.errors.emailInvalid', 'Email no válido');
    }
    return newErrors;
  };
  ...
  const handleImageChange = (e) => {
    ...
    if (!file.type.startsWith('image/')) {
      addNotification(t('profile.errors.invalidImage', 'Por favor selecciona una imagen válida'), 'error');
      return;
    }

    if (file.size > 5 * 1024 * 1024) {
      addNotification(t('profile.errors.imageTooLarge', 'La imagen no puede superar los 5MB'), 'error');
      return;
    }
  ...
  const handleUploadImage = async () => {
    if (!selectedImage) {
      addNotification(t('profile.errors.selectImageFirst', 'Por favor selecciona una imagen primero'), 'error');
      return;
    }
    ...
    try {
      ...
      addNotification(t('profile.avatarUpdated', 'Imagen de perfil actualizada correctamente'), 'success');
      ...
      addNotification(error.response?.data?.message || t('messages.error', 'Error al subir la imagen'), 'error');
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleSaveProfile = async (e) => {
    ...
    if (Object.keys(validationErrors).length > 0) {
      ...
      addNotification(t('profile.errors.correctErrors', 'Por favor corrige los errores'), 'error');
      return;
    }
    ...
    try {
      ...
      addNotification(t('profile.profileUpdated', 'Perfil actualizado correctamente'), 'success');
      updateUser({ ...user, ...formData });
      setErrors({});
    } catch (error) {
      console.error('Error updating profile:', error);
      const errorMsg = error.response?.data?.message || t('profile.errors.updateError', 'Error al actualizar el perfil');
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-400">
            {t('profile.subtitle', 'Gestiona tu información personal y preferencias')}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1">
            <div className="bg-gray-900/50 backdrop-blur-sm rounded-2xl border border-gray-800 p-6">
              <div className="flex flex-col items-center">
                <div className="relative mb-6">
                  <div className="w-40 h-40 rounded-full overflow-hidden border-4 border-gray-800">
                    {imagePreview ? (
                      ...
                      <img 
                        src={user.avatar_url} 
                        alt={t('profile.profilePicture', 'Perfil')} 
                        className="w-full h-full object-cover"
                      />
                    ) : (
                ...
                <h2 className="text-xl font-bold text-white mb-2">
                  {user?.name || t('profile.user', 'Usuario')}
                </h2>
                ...
                <div className={`px-4 py-2 rounded-full text-sm font-medium mb-6 ${
                  ...
                  {user?.role === 'admin' 
                    ? t('profile.role.admin', 'Administrador') 
                    : t('profile.role.user', 'Usuario')}
                </div>
                ...
                {selectedImage && !uploading && (
                  ...
                  >
                    ...
                    {t('profile.saveImage', 'Guardar imagen')}
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="mt-6 bg-gray-900/50 backdrop-blur-sm rounded-2xl border border-gray-800 p-6">
              <h3 className="text-lg font-semibold text-white mb-4">
                {t('profile.stats.title', 'Estadísticas')}
              </h3>
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-gray-400">
                    {t('profile.stats.gamesPlayed', 'Juegos jugados')}
                  </span>
                  <span className="text-white font-medium">0</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-gray-400">
                    {t('profile.stats.categoriesCreated', 'Categorías creadas')}
                  </span>
                  <span className="text-white font-medium">0</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-gray-400">
                    {t('profile.stats.memberSince', 'Miembro desde')}
                  </span>
          ...
          <div className="lg:col-span-2">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        {t('profile.fields.fullName', 'Nombre completo')}
                      </div>
                    </label>
                    <input
                      type="text"
                      ...
                      placeholder={t('profile.placeholders.yourName', 'Tu nombre')}
                    />
                    {errors.name && (
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        {t('profile.fields.preferredLanguage', 'Idioma preferido')}
                      </div>
                    </label>
            ...
            <div className="mt-6 bg-gray-900/50 backdrop-blur-sm rounded-2xl border border-gray-800 p-6">
              <h2 className="text-xl font-bold text-white mb-6">
                {t('profile.security.title', 'Seguridad')}
              </h2>
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-400">
                        {t('profile.security.changePasswordDescription', 'Actualiza tu contraseña regularmente para mantener la seguridad')}
                      </p>
                    </div>
                    <button className="px-4 py-2 bg-gray-800 hover:bg-gray-700 text-white rounded-lg font-medium transition-all">
                      {t('profile.security.change', 'Cambiar')}
                    </button>
                  </div>
                </div>
                ...
                <div className="p-4 bg-gray-800/30 rounded-lg border border-gray-700">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="font-medium text-white mb-1">
                        {t('profile.security.activeSessions', 'Sesiones activas')}
                      </h4>
                      <p className="text-sm text-gray-400">
                        {t('profile.security.activeSessionsDescription', 'Gestiona tus sesiones activas en otros dispositivos')}
                      </p>
                    </div>
                    <button className="px-4 py-2 bg-gray-800 hover:bg-gray-700 text-white rounded-lg font-medium transition-all">
                      {t('profile.security.viewSessions', 'Ver sesiones')}
                    </button>
                  </div>
                </div>
              </div>
